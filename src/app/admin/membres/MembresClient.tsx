...
export function MembresClient({ members }: { members: MemberRow[] }) {
        ...
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: "#1B3A2A", border: "1px solid #2A4A35" }}>
          {(["all", "debutant", "intermediaire", "avance"] as const).map((level) => (
            <button
              ...
              style={filterLevel === level ? { background: "#C9A84C", color: "#0F2318" } : { color: "#7A8A7A" }}
            >
        ...
        <div className="flex gap-1 p-1 rounded-xl" style={{ background: "#1B3A2A", border: "1px solid #2A4A35" }}>
          {(["all", "active", "inactive"] as const).map((s) => (
            <button
              ...
              className="px-3 py-1 rounded-lg text-xs font-medium transition-all"
              style={filterSub === s ? { background: "#C9A84C", color: "#0F2318" } : { color: "#7A8A7A" }}
            >
          ...
          filtered.map((member, idx) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[11px] font-medium px-2 py-1 rounded-lg w-fit" style={{ background: levelStyle.bg, color: levelStyle.text }}>
                ...
                <span className="text-sm font-semibold" style={{ color: "#F5F0E8" }}>{member.totalVideosWatched}</span>
